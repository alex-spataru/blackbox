...
typedef struct xTASK_STATUS
{
	...
	UBaseType_t uxBasePriority;		/* The priority to which the task will return if the task's current priority has been inherited to avoid unbounded priority inversion when obtaining a mutex.  Only valid if configUSE_MUTEXES is defined as 1 in FreeRTOScfg.h. */
	uint32_t ulRunTimeCounter;		/* The total run time allocated to the task so far, as defined by the run time stats clock.  See http://www.freertos.org/rtos-run-time-stats.html.  Only valid when configGENERATE_RUN_TIME_STATS is defined as 1 in FreeRTOScfg.h. */
 ...
 * configMAX_TASK_NAME_LEN in FreeRTOScfg.h.
 *
 ...
 *
 * INCLUDE_xTaskAbortDelay must be defined as 1 in FreeRTOScfg.h for this
 ...
 * must be set to 1 in FreeRTOScfg.h for pcTaskGetHandle() to be available.
 *
 ...
 *
 * INCLUDE_uxTaskGetStackHighWaterMark must be set to 1 in FreeRTOScfg.h for
...
constant. */
	...
	configNUM_THREAD_LOCAL_STORAGE_POINTERS setting in FreeRTOScfg.h.  The
 ...
 * INCLUDE_xTaskGetIdleTaskHandle is set to 1 in FreeRTOScfg.h.
 *
...
/**
 * configUSE_TRACE_FACILITY must be defined as 1 in FreeRTOScfg.h for
 ...
 * FreeRTOScfg.h then *pulTotalRunTime is set by uxTaskGetSystemState() to the
